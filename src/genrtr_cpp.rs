...
pub fn cpp_driver_code(examples: Vec<Example>, starter_code: &String) -> String {
    ...
    let function_name: String = String::from("func");
    ...
    if code_example_1.is_none() {
        ...
        for i in 1..examples.len() {
            driver_code += cpp_code_for_example(&examples[i], i + 1, starter_code)
                .unwrap()
                .as_str();
        }
    }
...
fn cpp_code_for_example(
    ...
    for i in 0..example.get_input_var_types().len() {
        let (input_var_name, input_var_type) = &example.get_input_var_types()[i];
        let input_var_value = &example.get_input_var_values()[input_var_name];
        let code_for_input_var = cpp_code_for_input_var(input_var_type, input_var_value);
    ...
    example_string += format!(
        "\t{} output_{} = {}(",
        example.get_output_type(),
        example_number,
        example.get_func_name()
    )
    .as_str();
    let mut i: usize = 0;
    ...
    // print output
    example_string += cpp_code_to_print_output(&example.get_output_type()).as_str();
...
fn cpp_code_for_input_var(lc_var_type: &String, var_value: &String) -> Option<(String, String)> {
    Some((lc_var_type.clone(), var_value.clone()))
}
...
fn cpp_code_to_print_output(lc_var_type: &String) -> String {
    lc_var_type.clone()
}
