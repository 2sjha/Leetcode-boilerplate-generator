...
pub fn java_driver_code(examples: Vec<Example>, starter_code: &String) -> String {
    let mut driver_code: String = format!("#include <iostream>\nusing namespace std;\n\n{}\n\nint main() {{\n\tSolution sol;\n\n", starter_code);
    let function_name: String = parser::parse_func_name_from_starter_code(starter_code);

    let code_example_1 = java_code_for_example(&examples[0], 1, &function_name);
    if code_example_1.is_none() {
        ...
        for i in 1..examples.len() {
            driver_code += java_code_for_example(&examples[i], i + 1, &function_name)
                .unwrap()
                .as_str();
        }
    }
...
fn java_code_for_example(
    ...
    for i in 0..example.get_input_var_types().len() {
        ...
        let code_for_input_var = java_code_for_input_var(lc_input_var_type, &input_var_value);
        if code_for_input_var.is_none() {
            return None;
        }

        let (java_input_var_type, java_input_var_value) = code_for_input_var.unwrap();
        example_string += format!(
            "\t{} {}_{} = {};\n",
            java_input_var_type, input_var_name, example_number, java_input_var_value
        )
        .as_str();
    }
    ...
    let code_for_output_var =
        java_code_for_output_var(&example.get_output_type(), &example.get_output_value());
    if code_for_output_var.is_none() {
        return None;
    }

    let (java_output_var_type, java_output_var_value) = code_for_output_var.unwrap();
    example_string += format!(
        "\t{} expected_{} = {};\n",
        java_output_var_type, example_number, java_output_var_value
    )
    .as_str();
    ...
    example_string += format!(
        "\t{} output_{} = sol.{}(",
        java_output_var_type, example_number, function_name
    )
    .as_str();
    let mut i: usize = 0;
...
fn java_code_for_input_var(lc_var_type: &String, var_value: &String) -> Option<(String, String)> {
    let java_var_type: String;
    let mut java_var_value: String;

    if lc_var_type == utils::IN_INT {
        java_var_type = String::from("int");
        java_var_value = var_value.clone()
    } else if lc_var_type == utils::IN_STRING {
        java_var_type = String::from("string");
        java_var_value = var_value.clone();
    } else if lc_var_type == utils::IN_LIST_INT {
        java_var_type = String::from("vector<int>");
        java_var_value = String::new();
        for c in var_value.chars() {
            if c == '[' {
                ...
                java_var_value.push(c);
            }
        }
    } else if lc_var_type == utils::IN_LIST_STRING {
        java_var_type = String::from("vector<string>");
        java_var_value = String::new();
        for c in var_value.chars() {
            if c == '[' {
                java_var_value.push('{');
        ...
        java_var_type = String::from("vector<vector<int>>");
        java_var_value = String::new();
        for c in var_value.chars() {
            if c == '[' {
                java_var_value.push('{');
        ...
        java_var_type = String::from("vector<vector<char>>");
        java_var_value = String::new();
        for c in var_value.chars() {
            if c == '[' {
                java_var_value.push('{');
    ...
    Some((java_var_type, java_var_value))
}

fn java_code_for_output_var(lc_var_type: &String, var_value: &String) -> Option<(String, String)> {
    let java_var_type: String;
    let mut java_var_value: String;
    ...
    if lc_var_type == utils::OUT_INT {
        ...
        java_var_type = String::from("bool");
        java_var_value = var_value.clone();
    } else if lc_var_type == utils::OUT_VOID {
        java_var_type = String::new();
        java_var_value = var_value.clone();
